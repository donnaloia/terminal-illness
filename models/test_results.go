...
func InitialResultsModel() ResultsModel {
	return ResultsModel{
		0, false, 10, 0, 0, false, false,
	}
}
...
func updateChoices(msg tea.Msg, m ResultsModel) (tea.Model, tea.Cmd) {
	switch msg := msg.(type) {
	case tea.KeyMsg:
		switch msg.String() {
		case "j", "down":
...
func updateChosen(msg tea.Msg, m ResultsModel) (tea.Model, tea.Cmd) {
	switch msg.(type) {
	case frameMsg:
		if !m.Loaded {
			m.Frames++
...
func choicesView(m ResultsModel) string {
	c := m.Choice
...
func chosenView(m ResultsModel) string {
	var msg string
